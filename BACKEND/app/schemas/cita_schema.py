...
class CitaSchema(Schema):
    ...
    agenda = fields.Nested(AgendaSchema, dump_only=True)
    agenda_id = fields.Int(load_only=True)
    motivo = fields.Str(required=True, validate=[validate.Length(max=250)])
    precio_cita = fields.Int(null=True)
    usuario = fields.Nested(UserTypeSchema, dump_only=True)
    usuario_id = fields.Int(load_only=True) 
    paciente = fields.Nested(MascotaTypeSchema, dump_only=True)
